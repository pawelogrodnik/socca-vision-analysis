...
- `[x]` Reviewed/public player report pokazuje opcjonalne tempo obciążenia na 5 minut czasu wykrytego: dystans, wysoka intensywność i sprinty, udział dystansu HI, szczegóły sprintów, najlepsze wiarygodne okno oraz macierz aktywności w kolejnych 5-minutowych oknach dostępnego nagrania.
...
**Status:** `[~]` background analysis job API/UI polling, persisted job status files, quality smoke checker, native runtime diagnostics, benchmark scripts, real chunked runner v1, admin production preflight/presets, review readiness guards, expanded pitch-edge detection ROI, light camera-motion compensation and package contract `0.2.0` are implemented. Stop controls, artifact cleanup, richer export/share polish and production-grade chunk merge polish are still pending.
...
### US12.6 — Native runtime benchmark
Jako developer chcę uruchomić ten sam benchmark na MPS/CUDA/CPU bez Dockera, żeby porównać realną wydajność laptopów przed analizą pełnych meczów.
...
- `[x]` Backend potrafi zapisać `analysis_chunk_manifest.json` z podziałem filmu na chunki, overlapem i statusem każdego chunka.
- `[x]` Chunked mode wykonuje realne per-chunk YOLO, pomija ukończone chunki przy retry/resume, scala tracki z globalnymi frame/time i uruchamia finalny stable resolver/statystyki na merged tracks.
...
- `[x]` UI ma presety analizy `fast_debug`, `standard_full`, `production_ball` i `quality_full`, które ustawiają payload analizy zamiast ręcznego klikania wszystkich pól.
  - `[x]` Admin UI ma review readiness checklist i blokuje package/publish, gdy brakuje wymaganych dokumentów tracking-only.
  - `[x]` YOLO player filtering używa `pitch_filter_margin_px` przy krawędziach boiska i clampuje pozycje boiskowe do prawdziwych wymiarów pitch config.
  - `[x]` Analiza YOLO ma opcjonalną kompensację lekkiego ruchu kamery: `camera_motion_report.json`, `camera_motion_overlay.mp4`, dynamiczny polygon boiska w overlayach oraz `calibrated_footpoint`/`calibrated_position_px` dla statystyk zawodników i piłki.
  - `[x]` `match_package.json` ma kontrakt `0.2.0` z sekcjami `required`, `optional`, `debug` oraz `package_validation`.
- `[x]` `publish-local` i publish blokują paczki z `package_validation.status === blocked`; ball/possession/pass pozostają opcjonalne.
