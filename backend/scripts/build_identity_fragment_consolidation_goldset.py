...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build and evaluate a versioned P1.8 fragment consolidation goldset.",
    )
    parser.add_argument("--reviewed-manifest", type=Path, action="append", required=True)
    parser.add_argument("--goldset-id", default="player-identity-fragment-consolidation")
    ...
    if args.prediction:
        evaluation = evaluate_identity_fragment_consolidation_goldset(
            goldset,
