...
from app.services.identity_reviewed_output_qa import (
    artifact_tree_snapshot,
    build_reviewed_output_qa,
)
...
def main() -> int:
    ...
    match = json.loads((options.match_root / "match.json").read_text(encoding="utf-8")); production_before = production_identity_snapshot(options.match_root, match); published_root = options.match_root.parent.parent / "published"; published_before = artifact_tree_snapshot(published_root); snapshot = finalize_reviewed_identity(options.match_root, match)
    ...
    if job.get("status") == "completed":
        qa = build_reviewed_output_qa(
            ...
            published_after=artifact_tree_snapshot(published_root),
        )
