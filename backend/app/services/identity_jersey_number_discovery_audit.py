...
function render(){{const item=current(),r=decision(item);draftBox=r.number_panel_bbox_normalized||null;selected=null;document.getElementById('meta').textContent=`#${{item.audit_index}} / ${{audit.items.length}} | klatka ${{item.frame}} | Team ${{item.team_label}} | wczesniejsza etykieta ukryta`;const n=done(),c=confirmed(),goal=Number(audit.target_confirmations||audit.summary?.target_confirmations||0);document.getElementById('progressText').textContent=`Przejrzane: ${{n}}/${{audit.items.length}}`;document.getElementById('confirmationText').textContent=goal?`Pewne numery: ${{c}}/${{goal}}${{c>=goal?' - cel osiagniety':''}}`:`Pewne numery: ${{c}}`;document.getElementById('confirmationText').style.color=c>=goal?'#28c76f':'#ffd85a';document.getElementById('progressBar').style.width=`${{100*n/audit.items.length}}%`;document.getElementById('previous').disabled=index===0;document.getElementById('next').disabled=index===audit.items.length-1;renderState();if(!item.image_filename){{canvas.width=800;canvas.height=520;ctx.fillStyle='#020817';ctx.fillRect(0,0,800,520);return;}}image=new Image();image.onload=()=>{{canvas.width=image.naturalWidth;canvas.height=image.naturalHeight;const fit=Math.min((wrap.clientWidth-40)/image.naturalWidth,(wrap.clientHeight-40)/image.naturalHeight);const readable=Math.min(12,680/image.naturalHeight);const z=Math.max(1,Math.min(20,Math.max(fit,readable)));canvas.style.width=Math.round(image.naturalWidth*z)+'px';canvas.style.height=Math.round(image.naturalHeight*z)+'px';draw();}};image.src=item.image_filename;saveLocal();}}
