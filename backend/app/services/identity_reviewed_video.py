...
def render_reviewed_video(match_path: Path, snapshot: dict[str, Any], match_doc: dict[str, Any], *, include_minimap: bool = True, include_ball: bool = True, show_roster_number: bool = False) -> dict[str, Any]:
    ...
    config={"include_minimap":include_minimap,"include_ball":include_ball,"show_roster_number":show_roster_number}; manifest={"schema_version":"1.0.0","status":"completed","generated_at":datetime.now(timezone.utc).isoformat(),"source_snapshot_digest":snapshot["semantic_digest"],"source_video_digest":_sha(source),"render_config_digest":canonical_digest(config),"renderer_version":"reviewed_video:v2","path":"reviewed_video.mp4","frames":count,"fps":fps,"resolution":[width,height],"duration_sec":round(count/fps,3),"file_size_bytes":output.stat().st_size,"digest":_sha(output),"render_duration_sec":round(time.monotonic()-started,3),"real_time_factor":round((count/fps)/max(time.monotonic()-started,.001),3),"minimap":minimap,"safety":{"reran_yolo":False,"reran_tracking":False,"production_identity_mutated":False}}
...
def reviewed_source_video_path(match_path: Path, match_doc: dict[str, Any]) -> Path:
    preferred = match_path / str(match_doc.get("video_filename") or "video.mp4")
    source = preferred if preferred.exists() else match_path / "video.mp4"
...
def _positions_by_frame(path: Path, snapshot: dict[str, Any]) -> dict[int, list[dict[str, Any]]]:
    ...
    for assignment in snapshot.get("tracklet_assignments") or []:
        ...
        for position in tracklets.get(str(assignment["tracklet_id"]),{}).get("positions_m") or []:
            ...
            if isinstance(bbox,list) and len(bbox)>=4: out[int(position.get("frame") or 0)].append({**position,**assignment})
    return out
...
def _hud(frame: Any, index:int,fps:float,rows:list[dict[str,Any]],snapshot:dict[str,Any])->None:
    import cv2
    summary=snapshot.get("summary") or {}; text=f"{index/fps:05.1f}s  Potwierdzone: {summary.get('confirmed_detected_frame_coverage') or 0:.0%}  Nieprzypisani: {sum(row.get('identity_status')=='unresolved' for row in rows)}"; cv2.rectangle(frame,(8,8),(min(frame.shape[1]-8,520),38),(10,14,20),-1); cv2.putText(frame,text,(16,29),cv2.FONT_HERSHEY_SIMPLEX,.52,(240,240,240),1,cv2.LINE_AA)
