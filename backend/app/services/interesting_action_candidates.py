...
MIN_SCORE = 0.55
MIN_CONFIDENCE = 0.35
MIN_TRUSTED_BALL_CONFIDENCE = 0.5
MIN_BALL_PROGRESS_DURATION_SEC = 1.0
MIN_BALL_FORWARD_PROGRESS_M = 7.0
BALL_PROGRESS_ENDPOINT_WINDOW_SEC = 0.5
...
def build_logical_candidate_signals(sources: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for source_index, source in enumerate(sources):
        ...
        possession_frames = _rows(source.get("possession_frames"))
        team_by_label = _team_ids_by_label(segments, _rows(source.get("pass_candidates")), possession_frames)
        ...
        signals.extend(
            _ball_progression_signals(
                segments,
                possession_frames,
                _rows(source.get("match_phase_periods")),
                team_by_label,
                offset=offset,
                source_id=source_id,
            )
...
def _candidate_from_seed(seed: Mapping[str, Any], signals: list[dict[str, Any]], gaps: list[tuple[float, float]], span: float) -> dict[str, Any] | None:
    ...
    ball_progressions = [row for row in relevant if row["kind"] == "ball_progression"]
    progressive = [row for row in passes if bool(row.get("progressive"))]
    pass_forward = sum(_number(row.get("forward_progress_m"), 0.0) for row in passes)
    ball_forward = max((_number(row.get("forward_progress_m"), 0.0) for row in ball_progressions), default=0.0)
    forward = max(pass_forward, ball_forward)
    ...
    # subsequent pass, observed ball progression, or momentum observation.
    developed = bool(passes or ball_progressions or momentum_rise >= 0.15 or (seed["kind"] == "pass" and forward >= 7.0))
    if not developed:
        return None
    ...
    if ball_progressions:
        strongest_ball_progression = max(ball_progressions, key=lambda row: _number(row.get("forward_progress_m"), 0.0))
        evidence.append({
            "kind": "ball_progression",
            "forward_progress_m": round(_number(strongest_ball_progression.get("forward_progress_m"), 0.0), 3),
            "progression_rate_mps": round(_number(strongest_ball_progression.get("progression_rate_mps"), 0.0), 3),
            "trusted_sample_count": int(_number(strongest_ball_progression.get("trusted_sample_count"), 0.0)),
        })
    if momentum_rise > 0:
        evidence.append({"kind": "momentum_rise", "delta": round(momentum_rise, 4)})
    if has_restart:
        evidence.append({"kind": "restart"})
    latest = max(float(row["end_time_sec"]) for row in relevant if row["kind"] in {"pass", "ball_progression", "momentum_rise", "restart"})
    end = min(horizon, max(start + 3.0, latest + 3.0))
    progression_score = min(0.35, forward / 20.0 * 0.35)
    score = 0.16 + (0.24 if seed["kind"] == "regain" else 0.08) + progression_score + min(0.15, len(progressive) * 0.075) + (0.08 if ball_progressions else 0.0) + min(0.12, momentum_rise * 0.18) + (0.08 if has_restart else 0.0)
    confidence_rows = [float(row["confidence"]) for row in relevant if row["kind"] in {"control", "pass", "ball_progression", "regain", "restart"}]
...
def _ball_progression_signals(
    segments: list[dict[str, Any]],
    possession_frames: list[dict[str, Any]],
    periods: list[dict[str, Any]],
    team_by_label: Mapping[str, str],
    *,
    offset: float,
    source_id: str,
) -> list[dict[str, Any]]:
    """Return only directly observed, direction-aware controlled ball progressions."""

    signals: list[dict[str, Any]] = []
    for segment in segments:
        start, end = _interval(segment)
        if start is None or end - start < MIN_BALL_PROGRESS_DURATION_SEC or str(segment.get("status") or "") != "controlled":
            continue
        team_id = _team_id(segment, team_by_label)
        direction = _attack_direction_at(periods, str(segment.get("team_label") or ""), start)
        if not team_id or direction not in {"towards_y_min", "towards_y_max"}:
            continue
        samples = [
            row for row in possession_frames
            if str(row.get("status") or "") == "controlled"
            and _team_id(row, team_by_label) == team_id
            and start <= _number(row.get("time_sec"), -1.0) <= end
            and str(row.get("ball_source") or "") == "detected"
            and _number(row.get("ball_confidence"), 0.0) >= MIN_TRUSTED_BALL_CONFIDENCE
            and _position_y(row.get("ball_position_m")) is not None
        ]
        first_y = [_position_y(row.get("ball_position_m")) for row in samples if _number(row.get("time_sec"), -1.0) <= start + BALL_PROGRESS_ENDPOINT_WINDOW_SEC]
        last_y = [_position_y(row.get("ball_position_m")) for row in samples if _number(row.get("time_sec"), -1.0) >= end - BALL_PROGRESS_ENDPOINT_WINDOW_SEC]
        if not first_y or not last_y:
            continue
        initial_y, final_y = median(first_y), median(last_y)
        forward = initial_y - final_y if direction == "towards_y_min" else final_y - initial_y
        if forward < MIN_BALL_FORWARD_PROGRESS_M:
            continue
        duration = end - start
        confidence = sum(_number(row.get("ball_confidence"), 0.0) for row in samples) / len(samples)
        signals.append(
            _signal(
                "ball_progression",
                offset + start,
                ...
                confidence,
                source_id,
                forward_progress_m=round(forward, 3),
                progression_rate_mps=round(forward / duration, 3),
                trusted_sample_count=len(samples),
...
def _attack_direction_at(periods: list[dict[str, Any]], team_label: str, time_sec: float) -> str:
    for period in periods:
        start, end = _interval(period)
        directions = period.get("team_attack_directions")
        if start is not None and start <= time_sec <= end and isinstance(directions, Mapping):
            direction = directions.get(team_label)
            return direction if isinstance(direction, str) else ""
    return ""


def _position_y(value: Any) -> float | None:
    return _number(value[1], -1.0) if isinstance(value, list) and len(value) == 2 and _number(value[1], -1.0) >= 0 else None
