...
def audit_pass_upstream_root_causes(
    ...
    candidates = _shared_fix_candidates(counts, contact_findings, identity_findings)
...
def _audit_missed(row: Mapping[str, Any], windows: Mapping[str, Mapping[str, Any]], indexes: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    root = _miss_root_cause(row, trace)
    return {
        "population": "MISS",
        ...
        "evidence": {
            ...
            "ball_frame_count": len(trace["ball"]["nearby_frames"]),
        },
        "upstream_trace": trace,
    }


def _miss_root_cause(row: Mapping[str, Any], trace: Mapping[str, Any]) -> str:
    prior = str(row.get("root_cause") or "")
    if prior == "EXCLUDED_BY_RELEASE_POLICY":
        return "RELEASE_POLICY"
    if prior == "NO_SOURCE_CONTACT":
        return "CONTACT_GENERATION" if trace["ball"]["nearby_frames"] else "BALL_TRACK"
    skipped = row.get("skipped_contact_pairs") or []
    if any(isinstance(item, Mapping) and item.get("skip_reason") for item in skipped):
...
def _audit_team_errors(report: Mapping[str, Any], indexes: Mapping[str, Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for match in report.get("error_audit") or []:
        ...
        for role, contact_key, expected_key, actual_key in (
            ...
            root = _team_error_root_cause(str(expected), contact_evidence, trace)
            rows.append({
                "population": "TEAM",
                "role": role,
                ...
                "root_cause": root,
                "evidence": {
                    "stable_player_id": contact_evidence.get("stable_player_id"),
                    "stable_subject_id": contact_evidence.get("stable_subject_id"),
...
def _team_error_root_cause(expected_team: str, contact: Mapping[str, Any], trace: Mapping[str, Any]) -> str:
    ...
    if _team_name(operator) == expected_team and _team_name(_mapping(contact.get("event_team"))) != expected_team:
    ...
    event_team = _team_name(_mapping(contact.get("event_team")))
    canonical_team = _team_name(_mapping(contact.get("canonical_subject_team")))
    if event_team and canonical_team and event_team != canonical_team:
        return "WRONG_GLOBAL_IDENTITY_SLOT" if canonical_team != expected_team else "WRONG_CONTACT_PLAYER"
    return "UNKNOWN"
...
def _audit_false_positive_sample(
    ...
    for row in selected:
        ...
        output.append({
            ...
            "root_cause": _fp_root_cause(classification),
            "evidence": {
...
def _fp_root_cause(classification: str) -> str:
    return {
        "SHOT_AS_PASS": "SHOT_REBOUND_CHAIN",
        "INTERVENTION_AS_PASS": "INTERVENTION_CHAIN",
        "CONTROL_AS_PASS": "CONTINUED_CONTROL_FRAGMENTATION",
...
def build_upstream_trace(
    ...
    contacts = _mapping(source.get("events"))
    selected_contact = _mapping(contacts.get(str(contact_event_id or "")))
    nearby_frames = [
        row for row in source.get("possession") or []
        if abs(_number(row.get("time_sec")) - source_time_sec) <= TRACE_RADIUS_SEC
    ]
    selected_subject = str(selected_contact.get("stable_subject_id") or "")
    closer_expected = _closer_team_alternatives(selected_contact, nearby_frames, expected_team)
    nearby_events = _nearby_rows(source.get("events") or {}, source_time_sec)
    nearby_passes = _nearby_rows(source.get("passes") or {}, source_time_sec)
    return {
        "source_match_id": source_match_id,
        "source_time_sec": _round(source_time_sec),
        "ball": {
            "nearby_frames": [
                {key: row.get(key) for key in ("frame", "time_sec", "ball_source", "ball_confidence", "ball_position_m", "status")}
                for row in nearby_frames[:12]
            ],
        },
        "nearby_players": [
            player for row in nearby_frames[:6] for player in row.get("nearest_players") or []
        ][:12],
        "selected_player": _contact_player_public(selected_contact),
        "team_assignment": _team_public(selected_contact),
        ...
        "contact": _contact_public(selected_contact),
        "event": _contact_public(selected_contact),
        "pass": nearby_passes,
...
def _nearby_rows(rows: Mapping[str, Mapping[str, Any]], time_sec: float) -> list[dict[str, Any]]:
    return [
        _contact_public(row)
...
def _contact_public(row: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        **{key: deepcopy(row.get(key)) for key in (
            "event_id", "candidate_id", "start_time_sec", "end_time_sec", "release_time_sec", "stable_player_id", "stable_subject_id",
            "team_label", "team_id", "team_name", "source_event_id", "target_event_id", "confidence", "review_status",
        )},
        "evidence_summary": {
...
def _ball_findings(missed: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    no_ball = sum(row.get("root_cause") == "BALL_TRACK" for row in missed)
    return {
        "misses_with_no_nearby_effective_ball_frame": no_ball,
        "ball_track_not_causal_for_remaining_misses": len(missed) - no_ball,
        "conclusion": "No ball-detector change is proposed; effective ball tracks are only marked causal where the trace contains no local usable frame.",
    }


def _shared_fix_candidates(
    counts: Mapping[str, Any], contact: Mapping[str, Any], identity: Mapping[str, Any],
) -> list[dict[str, Any]]:
    """Be intentionally strict: candidates need measured cross-population support."""

    total = _mapping(counts.get("total"))
    candidates: list[dict[str, Any]] = []
    if int(contact.get("wrong_possession_owner_demonstrated") or 0) and int(total.get("WRONG_POSSESSION_OWNER") or 0) >= 3:
        candidates.append({
            "root_cause": "WRONG_POSSESSION_OWNER",
            "production_layer": "ball_possession contact owner selection",
            "support": int(total["WRONG_POSSESSION_OWNER"]),
            "status": "requires_cross_population_review",
        })
    return candidates


def _go_no_go_decision(candidates: Sequence[Mapping[str, Any]]) -> str:
    return "CLEAR_SHARED_UPSTREAM_FIX" if candidates else "NO_SHARED_FIX_FOUND"
...
def render_pass_upstream_root_cause_audit_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines.extend(["", "## Contact-player association findings", "", f"- `{report.get('contact_player_association_findings')}`", "", "## Contact-fragmentation findings", "", f"- `{report.get('contact_fragmentation_findings')}`", "", "## Identity findings", "", f"- `{report.get('identity_findings')}`", "", "## Ball-track findings", "", f"- `{report.get('ball_track_findings')}`", "", "## Shared-fix candidates", ""])
    candidates = report.get("shared_fix_candidates") or []
    if candidates:
        lines.extend(f"- `{candidate}`" for candidate in candidates)
    else:
        lines.append("- None. No measured cause has a bounded, cross-population, gold-independent production fix.")
