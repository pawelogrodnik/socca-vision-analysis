    ...
    <section className="app">
      <section className="card">
        <div className="row between">
          <div>
            <h2>5. Identity candidates → player_id</h2>
            <p className="muted">Preferowany workflow dla dłuższych filmów: przypisuj kandydatów, nie pojedyncze raw tracker IDs.</p>
          </div>
        ...
        <label>
          ...
          <select value={selectedId} onChange={(event) => setSelectedId(event.target.value)}>
            ...
            {matches.map((match) => (
              <option key={match.id} value={match.id}>{match.title} · {match.match_date || 'brak daty'} · {match.status || 'uploaded'}</option>
            ))}
          </select>
        </label>
      </section>
      {selected ? (
        ...
        <section className="card"><p className="muted">Wybierz mecz, żeby zobaczyć identity candidates.</p></section>
      )}
    </section>
  );
}
