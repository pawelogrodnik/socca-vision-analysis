...
export function RedesignedReportVideoMoments({
  ...
  return <>
    ...
    <section className={`redesign-section redesign-video-section${expanded ? ' expanded' : ''}`} aria-labelledby={expanded ? undefined : 'redesign-moments-title'} role={expanded ? 'dialog' : undefined} aria-modal={expanded || undefined} aria-label={expanded ? 'Rozszerzona analiza meczu' : undefined}>
      ...
      {operatorEnabled ? <div className='key-moment-operator-tabs' role='tablist' aria-label='Tryb edycji momentów'><button type='button' role='tab' aria-selected={tab === 'accepted'} onClick={() => { setForm(null); setTab('accepted'); }}>Zaakceptowane {operatorMoments.length}</button><button type='button' role='tab' aria-selected={tab === 'suggested'} onClick={() => { setForm(null); setTab('suggested'); }}>Sugestie {editorState?.suggestions?.unreviewed_count ?? 0}</button></div> : null}
