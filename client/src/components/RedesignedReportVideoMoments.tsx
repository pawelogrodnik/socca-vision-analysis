...
import { PublicCanonicalShots } from './PublicCanonicalShots';
import { PublicShotMaps } from './PublicShotMaps';
...
export function RedesignedReportVideoMoments({
  ...
  const publicShots = report.shots;
  const publicShotProjectionAvailable = Array.isArray(publicShots);
  ...
  if (!configuredEmbedUrl && !publicMoments.length && !publicShotProjectionAvailable && !operatorEnabled && !externalVideoLoading) return null;
  ...
  return <>
    ...
    <section className={`redesign-section redesign-video-section${expanded ? ' expanded' : ''}`} aria-labelledby={expanded ? undefined : 'redesign-moments-title'} role={expanded ? 'dialog' : undefined} aria-modal={expanded || undefined} aria-label={expanded ? 'Rozszerzona analiza meczu' : undefined}>
      ...
      {!form && !shotForm && ((operatorEnabled && keyMomentOperatorEnabled && shotOperatorEnabled) || (!operatorEnabled && publicShotProjectionAvailable)) ? <div className='key-moment-operator-tabs review-domain-tabs' role='tablist' aria-label='Tryb analizy wideo'><button type='button' role='tab' aria-selected={domain === 'moments'} onClick={() => setDomain('moments')}>Momenty</button><button type='button' role='tab' aria-selected={domain === 'shots'} onClick={() => setDomain('shots')}>Strzały</button></div> : null}
