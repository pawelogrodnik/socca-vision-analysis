...
import { hasPublicShotMapLocation, publicShotOutcomeLabels, publicShotSummary, publicShotsForTeam } from '../lib/publicShotPresentation';
      ...
      {teams.map((team) => {
        ...
        return <article className='public-shot-map-card' key={teamId}>
          ...
          <div className='public-shot-pitch' aria-label={`Mapa strzałów: ${teamName(team)}`}>
            ...
            {mapped.map((shot) => <Marker key={shot.shot_id} shot={shot} teamName={teamName(team)} playerName={shot.player_id ? playerNames.get(shot.player_id) : undefined} onPlayAt={onPlayAt} onSelect={setSelected} />)}
          </div>
